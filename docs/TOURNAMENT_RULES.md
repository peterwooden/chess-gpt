...
- **Match protocol:** Proposed 2026-08-08, extending the 2026-07-31 standard-start protocol; to be ratified by the group before the next tournament is pinned. Each tournament is configured at creation either to start every game from the standard chess position (the 2026-07-31 protocol, unchanged) or to sample openings from the arena's fixed opening book (`site/lib/opening-book.mjs`). With the book on, the arena samples half as many openings as games-per-pair, at varying ply depths, when the tournament leaves registration — after entries are frozen, so no version can be nominated against a known draw — and records the draw permanently on the tournament. Every pairing plays every sampled opening twice, once with each color: games 2i and 2i+1 share opening i, so games per pair must be even. The runner plays the book moves itself before either package moves; they cost no thinking time, are recorded in the PGN (with an `Opening` header), and reach packages as ordinary `history`, so the package interface is unchanged and existing packages remain valid. The book exists because both published candidates select moves deterministically, so a standard-start pairing produces exactly two distinct games regardless of how many are scheduled. Further variation within an opening still comes only from stochasticity a submission chooses to implement, using the supplied deterministic `random()`. Whether the tournament that decides the title uses the book is part of ratifying this rule.
...
- **Game length:** Games end only under the ordinary rules of chess, including checkmate, stalemate, threefold repetition, the fifty-move rule, and insufficient material.
...
On 2026-08-07 the arena began serving `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: require-corp`, which makes `SharedArrayBuffer` available and lets ONNX Runtime Web run WASM inference on up to four threads instead of the single thread it had silently fallen back to. On the reference machine this took one measured position from 23.8 ms to 8.7 ms. This is runner infrastructure, not a rule change: it applies equally to every submission and no package needs editing. It does change what a package can afford under `moveTimeLimitMs`, so a search tuned against the old single-threaded runner will now finish early and leave depth unused. Results from before and after this date are therefore not directly comparable, and the change should be raised with the group before the next tournament is pinned.

On 2026-08-08 that isolation change was rolled back for the Sites deployment. Sites serves compiled worker assets ahead of the application Worker without retaining the worker response's embedder policy, so Chrome rejected the package worker before it could start. The production arena therefore runs ONNX Runtime Web on one WASM thread again. Results from the brief four-thread interval must remain separate from both earlier and later single-thread results.
