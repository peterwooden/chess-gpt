  ...
  return (
    ...
    >
      ...
      {visibleArrows.map((mark) => {
        ...
        const shape = thinkingArrowShape(mark.from, mark.to, orientation);
        return (
          <g key={mark.key} opacity={opacity}>
            <polyline
              className={`thinking-arrow ${mark.side}`}
              points={toSvgPoints(shape.shaft)}
              stroke={color}
            />
            <polygon
              className="thinking-arrow-head"
              points={toSvgPoints(shape.head)}
              fill={color}
            />
          </g>
        );
