...
6. **Hardware: trial then commit — a100-large, h200, rtx-pro-6000, l40sx1** (~$2–3 total). One ~10-min job per flavor at the exact final config on real shard data, measuring steady-state pos/s under default compile and `mode="reduce-overhead"`, NaN tripwire armed so the winner's trial is also the Exp-61 stability smoke. Selection: fastest wall-clock within 1.5× the cheapest $/FLOP. Expectation: h200 (4.8 TB/s) wins a bandwidth-bound workload at ~2–2.5× A100 throughput; full run ≈ 2–2.5 h ≈ $11. These trials plus the budget run are the **only** cloud compute in this phase — all pilots (scale, dropout, nobullet gate, second seed) are cancelled.
...
8. **Nomination: val curves + probe suite only — no gate games.** **[override, confirmed twice]** The teacher's recommendation was a 200-game local CPU gate vs cap64 (the 55/f2/67b inversion record, plus breakage detection); the learner deliberately accepts the inversion risk and nominates on curves, the package validator, and tactical probes. cap64/capstone-59 remain registered as fallbacks; nomination can be revisited any time before registration closes since entries are pinned only at freeze.
