...
## Run 68 thinking display — entrypoint-only republish (2026-08-08)

The 2026-08-08 rules revision added an optional `thinking` display to `chooseMove`, so run 68 got a second published package that uses it. **Weights are not involved**: `model.onnx` and `vocabulary.json` are copied byte for byte from `chess-gpt-fullbudget-68` (`lab/repackage.py`, new — the checkpoint is no longer local, and re-exporting ONNX to change only the adapter would risk a different model digest for nothing). The adapter is forked to `adapters/lab-fullbudget-thinking/`, following the `lab-beam` → `lab-beam-tc` precedent for interface variants; search logic is untouched.

What it draws: **candidate arrows** for the value-screened root moves that earned deepening, `intensity` by beam rank (best 1.0 → last 0.4, because value-head scores across a beam sit in a band a few hundredths wide and a score-proportional ramp renders four identical arrows); **search highlights** on the destination squares each ply expands, fainter with depth; **quiescence highlights** on contested squares; and a full-intensity **final arrow** for the move returned. Every exit path from `chooseMove` routes through one `commit()`, so the board never shows a stale candidate. Emission is a no-op when the runner omits `thinking` — which is still the normal case, since the probe harness and the deployed arena both omit it.

Budgets: at most 56 commands per move (two reserved so the final arrow survives budget exhaustion), measured peak 29 per move and 14 per 500 ms, against the runner limiter of 64/500 ms and the live-broadcast cap of 64 per batch — an over-emitting package costs spectators whole batches, so the margin is deliberate.

Move-identity evidence: all 6 tactical probes plus the 300 ms budget probe chose byte-identical moves to the published package, including the tie-broken `Ngxe2`; a 5-position A/B at a 120 s limit (so `timeFor()` never binds and the tree always completes) was identical throughout. A same-position A/B at a 10 s limit *does* flip moves — but the published package flips between the same two moves when run against itself, so that is the time-managed search's pre-existing wall-clock sensitivity, not the display. A throwing emitter still returns a legal move. Package 39,060,829 bytes (+1,144), validator green.

**Published: `peterwooden/chess-gpt-fullbudget-68-thinking` @ `8efe79368f48bff6c14f3667549a5848914c098b`.** The registered run-68 entry is pinned by commit SHA and manifest digest and is unaffected. Not yet visible anywhere: the arena's worker→client plumbing exists but no board overlay renders the commands yet (separate WIP).
## PUCT vs beam — the search experiment, and insufficient-material (2026-08-08, evening)

Learner directive: pick the single most promising search method, test it fast. Choice: PUCT (AlphaZero-style MCTS) — batch-flat WASM removes the sequential penalty, the model's two heads are exactly its two required functions. lab/mcts.py, ~250 evals then 10s/move budgets, vs the arena-tuned beam, same fullbudget-68 weights both sides, parallel single-thread CPU processes.

**Results: PUCT 13–0 over the beam** (round 0: 1–0 at 250 evals; round 1: 8–0 at 10s, collapsing to 2 unique trajectories; round 2 v2: 4–0, all distinct). Iteration v2 (root-perspective contempt fix + early-stop time management) then went **1.5–2.5 vs v1** head-to-head: the contempt fix is a real correctness repair (v1 was draw-seeking at opponent-parity nodes), but early-stop lost strength — under a per-move clock unspent time evaporates, and with tree reuse "unnecessary" evals deepen the subtree carried into future moves. Three of four v1-v2 games drew by threefold — the progress-blindness signature persists under PUCT too.

**v3 = full-clock spend + contempt fix + mate-in-1 scan, no early-stop**, ported to JS with per-eval deadline checks (retiring the beam's coarse-batch overshoot risk by construction). Published as **`peterwooden/chess-gpt-insufficient-material` @ `590ff7774d63a6f26d62407b4f61189a027fb6b6`** (fullbudget-68 weights unchanged; the subagent's publish was blocked by its consent guardrail, republished from the main session where the learner's instruction lives). Validation ran post-publish per the learner's ordering. First test: 2 games vs minichess-gpt-v1-final on the arena.

## v3 PUCT thinking display — entrypoint-only republish (2026-08-08)

`insufficient-material` (the v3 PUCT adapter on the run-68 weights) got the same treatment as the beam package: `model.onnx` and `vocabulary.json` copied byte for byte via `lab/repackage.py`, search untouched, display added in `adapters/lab-fullbudget/entry-v3-thinking.source.js`. Provenance check first — `entry-v3.source.js` rebuilds to the published `entry.js` byte for byte under the same esbuild flags, so the delta really is display-only.

The display differs from the beam one because the search does. **Visit share drives intensity** (floor 0.35, scaled against the leading move): MCTS visit counts have real dynamic range and evolve through the move, so they can be shown honestly, where the beam's value-head scores span a few hundredths and forced the rank fallback. Tree reuse means the root usually carries visits from the opponent's turn, so candidates are drawn before the first simulation rather than after a screening pass. **Redraws are throttled to a bounded count across the clock, not a fixed interval**: at ~250 batch-1 simulations per 10s move, per-simulation emission would be ~4x the runner's 64-per-500ms limiter, and a fixed interval would breach the same caps on a longer clock instead. Measured 23–24 commands per move, peak 4 per 500ms.

Move-identity evidence, which needed a different instrument here — `fullspend` MCTS never "completes", so the beam trick of granting unlimited time proves nothing. The equivalent handle is the *bottom* of the range: the loop always runs at least one simulation before checking the deadline, so a limit below one evaluation is a fully deterministic one-simulation regime. At 1ms and 50ms all 5 positions matched exactly; all 6 tactical probes and the 300ms budget probe matched the published package, including `Bxe2` (the beam adapter picks `Ngxe2` there — a real MCTS-vs-beam difference, and each matches its own baseline); at a 10s clock both returned the same move on 8 consecutive runs. One 120ms position differed — and the published package alone splits `e3`/`Nf3` 2:6 over 8 runs at that limit, so that is its own sim-count sensitivity. Worth noting the contrast with the beam adapter: **this search is stable at the tournament clock** (8/8 identical at 10s) where the beam one flipped, because spending the whole budget converges the visit distribution rather than landing wherever the depth cutoff falls.

**Published: `peterwooden/chess-gpt-insufficient-material-thinking` @ `7a6019a50caf1d652691b7f4c78ed212bd38c99c`.** Mirrors the source package, which carries no `runrecord.json` — worth adding to both, since the rules require one per entry.
