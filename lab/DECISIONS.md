...
ConvNeXt-style chess CNN (7×7 depthwise, inverted bottleneck, GELU, SE, mean+max pooling, depthwise compass rays with learnable gates; 14.8M params) launched at capstone-matched budget (1.98×10¹⁷, same data/epoch). **Silicon lesson:** depthwise convs ran memory-bound on A100 at ~half projected throughput; HF's job timeout was not enforced; the run was cancelled at 7h43m (~$19) at ~75% of the epoch via tripwire. **Gate: partial CNN 0.5/20 vs capstone-59** (0W–1D–19L) — decisively outclassed even with the mid-schedule handicap acknowledged. Conclusions: (1) the small-scale per-FLOP tie did not survive 10× scale in match currency — regime-flip #5; (2) paper-FLOPs ≠ silicon-FLOPs, divergence is architecture-specific — pilot curves must be fitted in wall-dollars alongside FLOPs; (3) long jobs get self-enforced watchdogs. **Transformer lane confirmed as the sole primary for the scaling pilot; CNN lane archived** with its real wins banked (kernel autopsy, dose-response ladder, the gallery). Credits ≈ $15 remain.
