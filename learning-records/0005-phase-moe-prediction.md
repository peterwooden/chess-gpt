...
The learner predicts that, when both candidates are trained with matched data, optimizer settings, processed positions, and FLOPs, the phase-routed mixture-of-experts model will win 70% of its head-to-head games against a size-matched shared snapshot model, with 70% confidence. The stated assumption is that the shared encoder represents board state in an information-efficient form that the experts can use. The predicted failure mode is a router failure at tournament inference.
