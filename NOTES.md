...
- The placement diagnostic showed working intuition for a simple Python loop, gradient direction, learning-rate instability, validation-size and capacity trade-offs, and tensor shapes. It exposed two starting gaps: negative log-likelihood as a probability-sensitive loss, and the distinct roles of train, validation, and test data. Broader Python, terminal/Git, calculus, and linear-algebra comfort remain only lightly sampled.
